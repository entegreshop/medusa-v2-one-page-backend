...
// Step 3: GET page
function getPage(path, authenticatedCookies) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: '204.168.136.196',
      port: 8000,
...
async function run() {
  try {
    ...
    const dashboardHtml = await getPage('/', loginResult.cookies);
    
    console.log('Searching for application references or project links in HTML...');
    // Find all links containing project or application UUID
    const links = dashboardHtml.match(/href="([^"]+)"/g) || [];
    console.log(`Total links found: ${links.length}`);
    
    // Print unique matching links
    const uniqueLinks = [...new Set(links)];
    const projectLinks = uniqueLinks.filter(link => link.includes('project') || link.includes('application') || link.includes('fnjekbskvqux7jzy4rjs1yef'));
    console.log('Project/Application related links:', JSON.stringify(projectLinks, null, 2));

    // Print first 2000 chars of dashboard body to inspect
    console.log('First 1000 characters of dashboard:');
