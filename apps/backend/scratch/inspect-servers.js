...
async function run() {
  try {
    ...
    console.log('Fetching /servers page...');
    const html = await getPage('/servers', loginResult.cookies);
    
    fs.writeFileSync('servers-page.html', html);
    console.log('Saved html to servers-page.html');
    
    // Find all links containing server
    const links = html.match(/href="([^"]+)"/g) || [];
    console.log('\n--- Links in Servers Page ---');
    const uniqueLinks = [...new Set(links)].map(l => l.replace('href="', '').replace('"', ''));
    uniqueLinks.forEach(link => {
      if (link.includes('server') || link.includes('network')) {
        console.log(link);
      }
    });
  } catch (err) {
