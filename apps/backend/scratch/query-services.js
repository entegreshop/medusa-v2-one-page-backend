...
function postLivewireUpdate(csrfToken, authenticatedCookies, referer, snapshotStr) {
  return new Promise((resolve, reject) => {
...
async function run() {
  try {
    ...
    console.log('Calling loadServices via Livewire...');
    const res = await postLivewireUpdate(authCsrfToken, loginResult.cookies, newPath, selectSnapshot);
    console.log('Status:', res.statusCode);
    
    const parsed = JSON.parse(res.body);
    const returnVal = parsed.components[0].effects.returns;
    if (returnVal && returnVal[0]) {
      const data = returnVal[0];
      console.log('\n--- Git Based Applications ---');
      console.log(JSON.stringify(data.gitBasedApplications, null, 2));
      console.log('\n--- Docker Based Applications ---');
      console.log(JSON.stringify(data.dockerBasedApplications, null, 2));
    } else {
      console.log('No return value found in response. Response:', JSON.stringify(parsed, null, 2));
    }
  } catch (err) {
