...
async function run() {
  try {
    ...
    console.log('Fetching /destinations page...');
    const html = await getPage('/destinations', loginResult.cookies);
    
    fs.writeFileSync('destinations-page.html', html);
    console.log('Saved html to destinations-page.html');
    
    // Find all links containing destinations
    const links = html.match(/href="([^"]+)"/g) || [];
    console.log('\n--- Links in Destinations Page ---');
    const uniqueLinks = [...new Set(links)].map(l => l.replace('href="', '').replace('"', ''));
    uniqueLinks.forEach(link => {
      if (link.includes('destination') || link.includes('network')) {
        console.log(link);
      }
    });
  } catch (err) {
