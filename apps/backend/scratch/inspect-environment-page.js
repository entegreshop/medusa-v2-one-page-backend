...
async function run() {
  try {
    ...
    const envPath = '/project/gsj9j6mz2rgs26h109lg7dxg/environment/mibkvgjurgjsm9urusgca6lu';
    console.log(`Fetching environment page: ${envPath}`);
    const html = await getPage(envPath, loginResult.cookies);
    
    fs.writeFileSync('environment-page.html', html);
    console.log('Saved html to environment-page.html');
    
    // Find all links containing server or destination
    const links = html.match(/href="([^"]+)"/g) || [];
    console.log('\n--- Links in Environment Page ---');
    const uniqueLinks = [...new Set(links)].map(l => l.replace('href="', '').replace('"', ''));
    uniqueLinks.forEach(link => {
      if (link.includes('server') || link.includes('destination') || link.includes('network')) {
        console.log(link);
      }
    });

    // Let's also print Livewire snapshots in this page
    ...
    while ((match = snapshotRegex.exec(html)) !== null) {
      ...
      try {
        ...
        console.log(`\nComponent: ${snap.memo.name}`);
        if (snap.memo.name.includes('project.resource')) {
          console.log('Data keys:', Object.keys(snap.data));
          console.log('Parameters:', snap.data.parameters);
        }
      } catch (e) {}
    }
  } catch (err) {
